...
const InfiniteLoopSlider = (
	...
	container.className = "loop-slider";
...
const App = () => {
	const app = document.getElementById("AnimationParent");
	const tagList = document.createElement("div");
	tagList.className = "tag-list";
	app.appendChild(tagList);
	[...new Array(ROWS)].forEach((_, i) => {
		const tags = shuffle(TAGS).slice(0, TAGS_PER_ROW);
		InfiniteLoopSlider(tagList, {
			duration: random(DURATION - 5000, DURATION + 5000),
			reverse: i % 2,
			children: tags.map((tag) => {
				const tagElem = document.createElement("div");
				Tag(tagElem, { text: tag });
				return tagElem;
			}),
		});
	});
